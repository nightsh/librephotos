...
class VisiblePhotoManager(models.Manager):
    ...
    def get_queryset(self):
        return (super().get_queryset().filter(
            Q(hidden=False) & Q(aspect_ratio__isnull=False)
            & Q(deleted=False)))
...
class Photo(models.Model):
    ...
    clip_embeddings = ArrayField(models.FloatField(blank=True, null=True),
                                 size=512,
                                 null=True)
    ...
    def save(
        ...
        modified_fields = [
            field_name for field_name, value in self._loaded_values.items()
        ...
        if save_metadata and user.save_metadata_to_disk != User.SaveMetadataToDisk.OFF:
            self._save_metadata(
                modified_fields,
                user.save_metadata_to_disk ==
                User.SaveMetadataToDisk.SIDECAR_FILE,
            )
        return super().save(
    ...
    def _save_metadata(self, modified_fields=None, use_sidecar=True):
        ...
        if tags_to_write:
            util.write_metadata(self.image_paths[0],
                                tags_to_write,
    ...
    def _generate_clip_embeddings(self, commit=True):
        ...
        if not self.clip_embeddings and image_path:
            try:
                ...
                if commit:
                    self.save()
                util.logger.info("generated clip embeddings for image %s." %
                                 (image_path))
            except Exception:
        ...
        try:
            ...
            if self.search_captions:
                self.search_captions = (
                    self.search_captions + " , " +
                    " , ".join(res_places365["categories"] +
                               [res_places365["environment"]]))
            else:
                self.search_captions = " , ".join(
                    res_places365["categories"] +
                    [res_places365["environment"]])
            if commit:
                self.save()
            util.logger.info("generated places365 captions for image %s." %
                             (image_path))
        except Exception:
            util.logger.exception(
    ...
    def _generate_thumbnail(self, commit=True):
        ...
        if self.video and not doesVideoThumbnailExists("square_thumbnails",
                                                       self.image_hash):
            createAnimatedThumbnail(
                inputPath=self.image_paths[0],
                outputHeight=500,
    ...
    def _find_album_date(self):
        ...
        if self.exif_timestamp:
            ...
            if (possible_old_album_date is not None
                    and possible_old_album_date.photos.filter(
                        image_hash=self.image_hash).exists):
    ...
    def _extract_date_time_from_exif(self, commit=True):
        ...
        def exif_getter(tags):
            return get_metadata(self.image_paths[0],
                                tags=tags,
                                try_sidecar=True)
    ...
    def _geolocate_mapbox(self, commit=True):
        ...
        if not new_gps_lat or not new_gps_lon:
            return
        if (old_gps_lat == float(new_gps_lat)
                and old_gps_lon == float(new_gps_lon)
                and old_album_places.count() != 0 and self.geolocation_json):
            return
    ...
    def _extract_video_length(self, commit=True):
        if not self.video:
            return
        (video_length, ) = get_metadata(self.image_paths[0],
                                        tags=[Tags.QUICKTIME_DURATION],
                                        try_sidecar=True)
        logger.debug(
            f"Extracted rating for {self.image_paths[0]}: {video_length}")
    ...
    def _extract_rating(self, commit=True):
        (rating, ) = get_metadata(self.image_paths[0],
                                  tags=[Tags.RATING],
                                  try_sidecar=True)
        if rating is not None:
            # Only change rating if the tag was found
            logger.debug(
                f"Extracted rating for {self.image_paths[0]}: {rating}")
            self.rating = rating
            if commit:
    ...
    def _extract_faces(self, second_try=False):
        ...
        try:
            qs_unknown_person = api.models.person.Person.objects.filter(
                Q(name="unknown")
                | Q(name=api.models.person.Person.UNKNOWN_PERSON_NAME))
            ...
            if self.image_paths != []:
                ...
                logger.info("image {}: rescan face failed".format(
                    self.image_paths[0]))
            if not second_try:
                self._extract_faces(True)
            else:
                if self.image_paths != []:
                    logger.error("image {}: rescan face failed".format(
                        self.image_paths[0]))
                else:
                    logger.error("image {}: rescan face failed".format(self))

    def _add_to_album_thing(self):
        if (type(self.captions_json) is dict
