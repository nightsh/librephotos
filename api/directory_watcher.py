...
def is_valid_media(image_path):
    ...
    if isRawPicture(image_path):
        return True
    try:
        pyvips.Image.thumbnail(image_path,
                               10000,
                               height=200,
                               size=pyvips.enums.Size.DOWN)
        return True
    except Exception as e:
        util.logger.info("Could not handle {}, because {}".format(
            image_path, str(e)))
        return False
...
if os.name == "Windows":
    ...
    def has_hidden_attribute(filepath):
        try:
            return bool(
                os.stat(filepath).st_file_attributes
...
def handle_new_image(user, image_path, job_id):
    if is_valid_media(image_path):
        try:
            ...
            util.logger.info("job {}: handling image {}".format(
                job_id, img_abs_path))
            ...
            if not Photo.objects.filter(Q(image_hash=image_hash)).exists():
                ...
                elapsed = (datetime.datetime.now() - start).total_seconds()
                util.logger.info("job {}: geolocate: {}, elapsed: {}".format(
                    job_id, img_abs_path, elapsed))
                ...
                elapsed = (datetime.datetime.now() - start).total_seconds()
                util.logger.info(
                    "job {}: add location to album dates: {}, elapsed: {}".
                    format(job_id, img_abs_path, elapsed))
                ...
                if photo.image_hash == "":
                    util.logger.warning(
                        "job {}: image hash is an empty string. File path: {}".
                        format(job_id, photo.image_path))
            else:
                photo = Photo.objects.filter(Q(image_hash=image_hash)).first()
                photo.image_paths.append(img_abs_path)
                photo.save()
                photo._check_image_paths()
                util.logger.warning("job {}: file {} exists already".format(
                    job_id, image_path))
        except Exception as e:
            try:
                ...
                util.logger.exception("job {}: could not load image {}".format(
                    job_id, image_path))
    ...
    try:
        if is_valid_media(image_path):
            photo = Photo.objects.filter(
                Q(image_paths__contains=image_path)).get()
            photo._generate_thumbnail(True)
            photo._calculate_aspect_ratio(False)
            photo._geolocate_mapbox(True)
            photo._extract_exif_data(True)
...
def _file_was_modified_after(filepath, time):
    try:
        ...
        return False
    return datetime.datetime.fromtimestamp(modified).replace(
...
def photo_scanner(user, last_scan, full_scan, path, job_id):
    if Photo.objects.filter(image_paths__contains=path).exists():
        ...
        if (full_scan or not last_scan or any([
...
def scan_photos(user, full_scan, job_id, scan_directory=""):
    if not os.path.exists(
        ...
        os.mkdir(
            os.path.join(ownphotos.settings.MEDIA_ROOT, "square_thumbnails"))
    ...
    try:
        ...
        last_scan = (LongRunningJob.objects.filter(finished=True).filter(
            job_type=1).filter(
                started_by=user).order_by("-finished_at").first())
        all = []
        ...
        if site_config.HEAVYWEIGHT_PROCESS > 1:
            ...
            num_threads = max(
                1,
                torch.get_num_threads() // site_config.HEAVYWEIGHT_PROCESS)
        ...
        util.logger.info("Scanned {} files in : {}".format(
            files_found, scan_directory))
    ...
    return {
        "new_photo_count": added_photo_count,
        "status": lrj.failed is False
    }
