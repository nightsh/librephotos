...
class AlbumPersonViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return (Person.objects.annotate(photo_count=Count(
            "faces", filter=Q(
                faces__photo__hidden=False), distinct=True)).filter(
                    Q(photo_count__gt=0)).prefetch_related(
                        Prefetch(
                            "faces",
    ...
    def retrieve(self, *args, **kwargs):
        ...
        serializer = GroupedPersonPhotosSerializer(
            queryset.filter(id=albumid).first())
        ...
        qs = (Person.objects.filter(
            ...
            & Q(faces__person_label_is_inferred=False)).distinct().annotate(
                viewable_face_count=Count("faces")).filter(
                    Q(viewable_face_count__gt=0)).order_by("name"))
        return qs
...
class AlbumThingViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return (AlbumThing.objects.filter(
            Q(owner=self.request.user) & Q(photos__hidden=False)).annotate(
                photo_count=Count("photos")).filter(
                    Q(photo_count__gt=0)).prefetch_related(
                        Prefetch(
                            "photos",
                            queryset=Photo.objects.filter(hidden=False).only(
                                "image_hash", "public", "rating", "hidden",
                                "exif_timestamp").order_by("-exif_timestamp"),
                        )))
...
class AlbumThingListViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return (AlbumThing.objects.filter(
            Q(owner=self.request.user) & Q(photos__hidden=False)).annotate(
                photo_count=Count("photos")).filter(
                    Q(photo_count__gt=0)).order_by("-title"))
...
class AlbumPlaceViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return (AlbumPlace.objects.annotate(photo_count=Count(
...
class AlbumPlaceListViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return (AlbumPlace.objects.filter(owner=self.request.user).annotate(
            photo_count=Count(
                "photos", filter=Q(
                    photos__hidden=False), distinct=True)).filter(
                        Q(photo_count__gt=0)
                        & Q(owner=self.request.user)).order_by("title"))
...
class AlbumUserListViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return (AlbumUser.objects.filter(owner=self.request.user).annotate(
            photo_count=Count(
                "photos", filter=Q(
                    photos__hidden=False), distinct=True)).filter(
                        Q(photo_count__gt=0)
...
class AlbumDateViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.query_params.get("person"):
            qs = AlbumDate.objects.filter(
                Q(owner=self.request.user)
                & Q(photos__hidden=False)
                & Q(photos__faces__person__id=self.request.query_params.get(
                    "person"))
        ...
        qs = (qs.annotate(photo_count=Count("photos")).filter(
    ...
    def retrieve(self, *args, **kwargs):
        queryset = self.get_queryset()
        albumid = re.findall(r"\'(.+?)\'", args[0].__str__())[0].split("/")[-2]
        serializer = PigAlbumDateSerializer(
            queryset.filter(id=albumid).first())
        serializer.context = {"request": self.request}
        return Response({"results": serializer.data})
...
class AlbumDateListViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.query_params.get("deleted"):
            qs = (AlbumDate.objects.filter(
                Q(owner=self.request.user) & Q(photos__deleted=True)).annotate(
                    photo_count=Count("photos", distinct=True)).filter(
                        Q(photo_count__gt=0)).order_by(
                            F("date").desc(nulls_last=True)))
            return qs
        if self.request.query_params.get("person"):
            return (AlbumDate.visible.filter(
                Q(owner=self.request.user)
                & Q(photos__faces__person__id=self.request.query_params.get(
                    "person"))
                & Q(photos__faces__person_label_is_inferred=False)
                ...
                Prefetch(
                    ...
                    queryset=Photo.visible.filter(
                        Q(faces__person__id=self.request.query_params.get(
                            "person"))
                        & Q(faces__person_label_is_inferred=False)).order_by(
                            "-exif_timestamp").only("image_hash", ).distinct(),
                ), ).annotate(
                    photo_count=Count("photos", distinct=True)).filter(
                        Q(photo_count__gt=0)).order_by(
                            F("date").desc(nulls_last=True)))

        qs = (qs.annotate(photo_count=Count("photos", distinct=True)).filter(
            Q(photo_count__gt=0)).order_by(F("date").desc(nulls_last=True)))
