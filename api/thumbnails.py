...
def createThumbnail(inputPath, outputHeight, outputPath, hash, fileType):
    if isRawPicture(inputPath):
        if "thumbnails_big" in outputPath:
            completePath = os.path.join(ownphotos.settings.MEDIA_ROOT,
                                        outputPath, hash + fileType).strip()
            ...
            bigThumbnailPath = os.path.join(ownphotos.settings.MEDIA_ROOT,
                                            "thumbnails_big", hash + fileType)
        ...
        x = pyvips.Image.thumbnail(inputPath,
                                   10000,
                                   height=outputHeight,
                                   size=pyvips.enums.Size.DOWN)
        completePath = os.path.join(ownphotos.settings.MEDIA_ROOT, outputPath,
                                    hash + fileType).strip()
        x.write_to_file(completePath)
        return completePath


def createAnimatedThumbnail(inputPath, outputHeight, outputPath, hash,
                            fileType):
    output = os.path.join(ownphotos.settings.MEDIA_ROOT, outputPath,
                          hash + fileType).strip()
    subprocess.call([
        "ffmpeg",
        "-i",
...
def createThumbnailForVideo(inputPath, outputPath, hash, fileType):
    subprocess.call([
        ...
        "-vframes",
        "1",
        os.path.join(ownphotos.settings.MEDIA_ROOT, outputPath,
                     hash + fileType).strip(),
    ])
